...
const _sfc_main$3 = /* @__PURE__ */ defineComponent({
  ...
  props: {
    tabData: {},
    placement: {},
    fullscreenPossible: { type: Boolean },
    fullscreenActive: { type: Boolean }
  },
  emits: ["move", "fullscreen"],
  setup(__props) {
    const mounted = ref(false);
    onMounted(() => mounted.value = true);
    return (_ctx, _cache) => {
      return mounted.value ? (openBlock(), createElementBlock("div", _hoisted_1$2, [
          ...
          withDirectives(createBaseVNode("div", _hoisted_2$2, [
            createBaseVNode("div", _hoisted_3$1, [
              withDirectives(createBaseVNode("button", {
                ...
                createVNode(unref(FIcon), {
                  name: __props.tabData.fullscreen ? "dash" : "new-window"
                }, null, 8, ["name"])
              ], 512), [
                [vShow, __props.fullscreenPossible]
              ]),
              createBaseVNode("button", {
                onClick: _cache[1] || (_cache[1] = ($event) => _ctx.$emit("move"))
              }, [
                createVNode(unref(FIcon), {
                  name: "caret-up",
                  rotate: __props.tabData.right ? "270" : "90"
                }, null, 8, ["rotate"])
              ])
            ]),
            ...
            [
              vShow,
              __props.tabData.active && (__props.tabData.fullscreen || !__props.fullscreenActive)
            ]
          ])
        ], 8, ["to"]))
      ])) : createCommentVNode("", true);
...
const _hoisted_6 = { class: "myrow content" };
let idCounter = 0;
const _sfc_main$2 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const flikar = ref(
      props.flikInfo.map((info, index) => {
        const isString2 = typeof info === "string";
        return {
          id: index + 1,
          heading: isString2 ? info : info.heading,
          active: !isString2 && !!info.active,
          right: !isString2 && !!info.right,
          fullscreen: false
        };
      })
    );
    ...
    const fullscreenFlik = computed(
      () => fullscreenPossible.value && flikar.value.find((flik) => flik.fullscreen) || void 0
    );
    function reorganize(movingFlik) {
    ...
    function onMove(flik) {
      reorganize(flik);
    }
    function onFullscreenChange(flik) {
      if (flik.fullscreen) {
        flik.fullscreen = false;
      } else {
        flikar.value.forEach((flik2) => flik2.fullscreen = false);
        flik.fullscreen = true;
      }
    }
    function onStartDragFlik(event, flikId) {
    ...
    function onDropFlik(event, right) {
      const flikId = parseInt(event.dataTransfer?.getData("FlikId") || "", 10);
      const flik = flikar.value.find((flik2) => flik2.id === flikId);
        ...
        createBaseVNode("div", _hoisted_1$1, [
          createBaseVNode("div", _hoisted_2$1, [
            (openBlock(), createElementBlock(Fragment, null, renderList([false, true], (right) => {
              return withDirectives(createBaseVNode("div", {
                key: right ? 0 : 1,
                class: "mycol",
                onDrop: ($event) => onDropFlik($event, right),
                onDragover: _cache[0] || (_cache[0] = withModifiers(() => {
                }, ["prevent"])),
                onDragenter: _cache[1] || (_cache[1] = withModifiers(() => {
                }, ["prevent"]))
              }, [
                (openBlock(true), createElementBlock(Fragment, null, renderList(right ? rightFlikar.value : leftFlikar.value, (flik) => {
                  return openBlock(), createElementBlock("div", {
                    key: flik.id,
                    class: normalizeClass(["heading2", {
                      active: flik.active,
                      shrouded: fullscreenFlik.value && fullscreenFlik.value.id !== flik.id
                    }]),
                    draggable: "true",
                    onClick: ($event) => activate(flik.id),
                    onDragstart: ($event) => onStartDragFlik($event, flik.id)
                  }, [
                    createTextVNode(toDisplayString(flik.heading) + " ", 1),
                    createBaseVNode("button", {
                      class: "move-button",
                      onClick: withModifiers(($event) => onMove(flik), ["stop"])
                    }, [
                      createVNode(unref(FIcon), {
                        name: "caret-up",
                ...
                [vShow, right ? rightFlikar.value.length : leftFlikar.value.length]
              ]);
            }), 64))
          ]),
          createBaseVNode("div", _hoisted_6, [
            withDirectives(createBaseVNode("div", {
              ...
              [vShow, leftFlikar.value.length && !fullscreenFlik.value?.right]
            ]),
            withDirectives(createBaseVNode("div", {
              id: rightId,
              class: "mycol"
              ...
              [
                vShow,
                rightFlikar.value.length && (!fullscreenFlik.value || fullscreenFlik.value.right)
              ]
            ])
          ])
        ]),
        (openBlock(true), createElementBlock(Fragment, null, renderList(flikar.value, (flik) => {
          return openBlock(), createBlock(FTab, {
            key: flik.id,
            "tab-data": flik,
            placement: flik.right ? rightId : leftId,
            "fullscreen-possible": fullscreenPossible.value,
            "fullscreen-active": !!fullscreenFlik.value,
            onMove: ($event) => onMove(flik),
            onFullscreen: ($event) => onFullscreenChange(flik)
          }, {
            default: withCtx(() => [
              renderSlot(_ctx.$slots, flik.id, {}, void 0, true)
            ]),
            _: 2
          }, 1032, ["tab-data", "placement", "fullscreen-possible", "fullscreen-active", "onMove", "onFullscreen"]);
        }), 128))
      ], 64);
    };
  }
    ...
    return (_ctx, _cache) => {
      return openBlock(), createElementBlock(Fragment, null, [
        ...
        createVNode(FTabHandler, { "flik-info": [
          ...
          { heading: "Flik 3", right: true },
          { heading: "Flik 4", active: true, right: true },
          "Flik 5"
        ] }, {
          "1": withCtx(() => [..._cache[0] || (_cache[0] = [
            createTextVNode(" Ett ensamt epos existerar evigt, emellertid erhålls endast ekot. ", -1),
            createBaseVNode("br", null, null, -1),
            createBaseVNode("input", { placeholder: "Fyll i data" }, null, -1)
          ])]),
          "2": withCtx(() => [..._cache[1] || (_cache[1] = [
            createTextVNode(" Två tappra troll tryggade tillvaron tilsammans. ", -1),
            createBaseVNode("br", null, null, -1),
            createBaseVNode("input", { placeholder: "Fyll i data" }, null, -1)
          ])]),
          "3": withCtx(() => [..._cache[2] || (_cache[2] = [
            createTextVNode(" Tre tysta träd trotsade tidens tand. ", -1),
            createBaseVNode("br", null, null, -1),
            createBaseVNode("input", { placeholder: "Fyll i data" }, null, -1)
          ])]),
          "4": withCtx(() => [..._cache[3] || (_cache[3] = [
            createTextVNode(" Fyra fina fåglar flyger fritt förbi fjärran fält. ", -1),
            createBaseVNode("br", null, null, -1),
            createBaseVNode("input", { placeholder: "Fyll i data" }, null, -1)
          ])]),
          "5": withCtx(() => [..._cache[4] || (_cache[4] = [
            createTextVNode(" Fem flitiga fingrar formar framtidens framgång. ", -1),
            createBaseVNode("br", null, null, -1),
            createBaseVNode("input", { placeholder: "Fyll i data" }, null, -1)
          ])]),
          _: 1
  ...
  setup(__props) {
    ...
    return (_ctx, _cache) => {
      return openBlock(), createElementBlock("div", _hoisted_1, [
        _cache[1] || (_cache[1] = createBaseVNode("h1", null, "FKUI Sandbox", -1)),
...
const routes = [{ path: "/", name: "", component: _sfc_main }];
